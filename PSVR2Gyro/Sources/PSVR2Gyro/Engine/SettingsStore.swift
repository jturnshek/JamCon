...
final class SettingsStore: @unchecked Sendable {
    ...
    struct InputSettings {
        ...
        var activeProfile: ControllerProfile = .psvr2Right

        // MARK: - Per-Type Gyro Settings
        var gyroSettings: [ControllerKind: GyroSettingsState] = [
            .psvr2: .defaultForKind(.psvr2),
            .joyCon: .defaultForKind(.joyCon)
        ]

        // MARK: - Per-Profile Button Mappings
        var psvr2ButtonMappings: [ControllerProfile: PSVR2ButtonMappingProfile] = [:]
        ...
        /// Current PSVR2 button mapping for the active profile
        var currentPSVR2ButtonMapping: PSVR2ButtonMappingProfile {
            get { psvr2ButtonMappings[activeProfile] ?? .load(for: activeProfile) }
            set { psvr2ButtonMappings[activeProfile] = newValue }
        }
        ...
        var buttonMappingProfile: PSVR2ButtonMappingProfile {
            get { currentPSVR2ButtonMapping }
            set { psvr2ButtonMappings[activeProfile] = newValue }
        }
        ...
        static func loadFromDefaults() -> InputSettings {
            ...
            // Load per-profile button mappings for PSVR2
            for profile in [ControllerProfile.psvr2Left, .psvr2Right] {
                settings.psvr2ButtonMappings[profile] = .load(for: profile)
            }
            ...
            let psvr2Profile = settings.psvr2ButtonMappings[.psvr2Right] ?? .load()
            settings.triggerThreshold = psvr2Profile.triggerThreshold
            settings.holdThreshold = psvr2Profile.holdThreshold
        ...
        private static func migrateSettings(from version: Int) {
            ...
            if version < 3 {
                ...
                let legacyPSVR2 = PSVR2ButtonMappingProfile.load()
                legacyPSVR2.save(for: .psvr2Left)
                legacyPSVR2.save(for: .psvr2Right)
