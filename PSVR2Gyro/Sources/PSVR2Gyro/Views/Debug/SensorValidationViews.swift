...
struct TriggerFeedbackView: View {
    let reportBytes: [UInt8]

    private var byte41: UInt8 { reportBytes.count > 41 ? reportBytes[41] : 0 }
    private var byte42: UInt8 { reportBytes.count > 42 ? reportBytes[42] : 0 }

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("Trigger Feedback Status")
                    .font(.headline)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Text("Byte 41:")
                            .font(.system(.body, design: .monospaced))
                            .frame(width: 70, alignment: .leading)
                        Text(String(format: "0x%02X", byte41))
                            .font(.system(.body, design: .monospaced))
                            .foregroundColor(.orange)
                        Text("= \(byte41)")
                            .font(.caption)
                    ...
                    HStack {
                        Text("Byte 42:")
                            .font(.system(.body, design: .monospaced))
                            .frame(width: 70, alignment: .leading)
                        Text(String(format: "0x%02X", byte42))
                            .font(.system(.body, design: .monospaced))
                            .foregroundColor(.orange)
                        Text("= \(byte42)")
                            .font(.caption)
            ...
            Text("Their claim: Motor location + adaptive trigger mode. May be DualSense-specific, not PSVR2 Sense.")
                .font(.caption)
                .foregroundColor(.secondary)
        }
        .padding()
...
struct CRC32ValidatorView: View {
    let reportBytes: [UInt8]

    private func computeCRC32(_ data: [UInt8]) -> UInt32 {
    ...
    private var reportedValue: UInt32 {
        guard reportBytes.count >= 78 else { return 0 }
        return UInt32(reportBytes[74]) |
               (UInt32(reportBytes[75]) << 8) |
               (UInt32(reportBytes[76]) << 16) |
               (UInt32(reportBytes[77]) << 24)
    }

    private var computedCRC: UInt32 {
        guard reportBytes.count >= 74 else { return 0 }
        return computeCRC32(Array(reportBytes[0..<74]))
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("CRC32 Validator")
                    .font(.headline)
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    Text(String(format: "0x%08X", reportedValue))
                        .font(.system(.body, design: .monospaced))
                        .foregroundColor(.orange)
                }
                ...
                HStack {
                    Text("CRC32(0-73):")
                        .font(.system(.body, design: .monospaced))
                        .frame(width: 100, alignment: .leading)
                    Text(String(format: "0x%08X", computedCRC))
                        .font(.system(.body, design: .monospaced))
                ...
                HStack {
                    Image(systemName: "xmark.circle.fill")
                        .foregroundColor(.red)
                    Text("Never matches - claim likely incorrect or different algorithm")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            }

            Text("Their claim was CRC32 checksum. May be different polynomial, byte range, or not a checksum at all.")
                .font(.caption)
                .foregroundColor(.secondary)
        }
        .padding()
